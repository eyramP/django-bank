...
class CustomTokenCreateView(TokenCreateView):
    def _action(self, serializer):
        ...
        if user.is_locked_out:
            return Response(
                {"error": f"Account is locked due to multiple failed login attemtps. Please try again after {settings.LOCKOUT_DURATION.total_seconds() / 60}"},
                status=status.HTTP_403_FORBIDDEN
            )
        user.reset_failed_login_attemtps()

        otp = generate_otp()
        user.set_otp(otp)
        send_otp_email(user.email, otp)
        logger.info(
            f"OTP sent for the login to user: {user.email}"
        )

        return Response(
            {
                "success": "OTP send to you email",
                "email": user.email
            },
    ...
    def post(self, request, *args: Any, **kwargs: Any) -> Response:
        ...
        try:
            ...
            if user:
                ...
                logger.error(
                    f"Failed login attempts: {failed_attempts} for user: {user.email}"
                )
                if failed_attempts >= settings.LOGIN_ATTEMPTS:
                    return Response(
                        {
                            "error": f"You have exceeded the maximum login attempts "
                                     f"You account has been locked for "
                                     f"{settings.LOCKOUT_DURATION.total_seconds() / 60} minutes"
                                     f"An email had been sent to you for further instructions"
                        },
                        status=status.HTTP_403_FORBIDDEN
                    )
            else:
...
class OTPVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if user.is_locked_out:
            return Response(
                {
                    "error": f"Account is locked dur to mutiple failed login attempt"
                             f"Please try again after "
                             f"{settings.LOCKOUT_DURATION.total_seconds() / 60} minuts"
                },
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        set_auth_cookies(response, access_token, refres_token, )
        logger.info(f"Successful logged in with OTP: {user.email}")
        return response
