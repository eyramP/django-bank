...
def sent_account_locked_email(self):
    ...
    try:
        email.send()
        logger.info(f"Account locked email send to {self.email}")
    except Exception as e:
        logger.error(f"Faile to sent account locked email to{self.email}: Error: {str(e)}")
