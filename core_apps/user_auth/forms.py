...
class UserCreationForm(DjangoUserCreationForm):
    ...
    def clean_email(self):
        email = self.clearn_data.get(email)
        if User.objects.filter(email=email).exists():
            raise ValidationError(_("A user with this email already exists in our records"))
        return email

    def clean_id_no(self):
        id_no = self.cleaned_data.get("id_no")
        if User.objects.filter(id_no=id_no).exists():
            raise ValidationError(_("A user with this ID number alreay exists."))
        return id_no
...
class UserChangeForm(DjangoUserChangeForm):
    ...
    def clean_id_no(self):
        ...
        if User.objects.exclude(pk=self.instance.pk).filter(id_no=id_no).exists():
            raise ValidationError(_("A user with this ID number already exist."))
        return id_no
