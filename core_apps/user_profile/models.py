    ...
    def clean(self) -> None:
        ...
        if self.id_issue_date and self.id_expiry_date:
            if self.id_expiry_date <= self.id_issue_date:
                ...
                _("ID expiry date must come after issue date.")
                )
...
class NextOfKin(TimeStampedModel):
    ...
    # The clean method is called before django saves an object in the db
    def clean(self) -> None:
        super().clean()
        if self.is_primary:
    ...
    def __str__(self) -> str:
        return f"{self.first_name} {self.last_name} - Next of king for {self.profile.user.full_name}"
