        ...
        if kyc_verified:
            if not verification_date:
                raise serializers.ValidationError(
                    _("Verification date is required when verifying account.")
                )

            if not verification_notes:
                raise serializers.ValidationError(
                    _("Verification notes are required when verifying account.")
                )
            ...
            if kyc_submitted and not all([
                ...
                raise serializers.ValidationError(
                    _("All verification fields (KYC Verified, verification date and notes) are required when submitting KYC.")
                )
